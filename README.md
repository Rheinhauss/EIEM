...
标准的左右足 IK 和脚尖 IK 会在主线程通过 Humanoid 两骨链解算，并遵守 VMD 的 IK 启用/禁用帧。由于直接模式不加载 PMX，IK 目标以 VMD 第一帧校准到角色当前脚位；PMX 专用链限制、模型局部轴、追加骨和物理骨语义仍不支持。非标准骨骼会计入「未支持」并写入日志，不会进行容易产生错误姿势的模糊匹配。模型比例特殊或依赖专用 IK 配置的动作仍建议使用 MUS4。

## 动作导出（VMD → MUS4）

MUS4 是保留的高精度预烘焙方案。直接 VMD 模式无需执行本节；如果动作依赖 PMX 专用 IK/局部轴，或直接重定向效果不理想，请通过 Unity 编辑器将 VMD 转换为 MUS4 格式的 `muscle_anim.bin`。
